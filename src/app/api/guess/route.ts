...
export async function POST(request: Request) {
  ...
  if (!pcodeRow)
    return NextResponse.json({ error: "No case assigned" }, { status: 404 });
  ...
  const matches = await db
    ...
    .where(
      and(
        ...
        or(eq(student.role, "senior"), eq(student.role, "house_leader")),
        isNull(student.deletedAt),
      ),
    );
  if (matches.length === 0)
    return NextResponse.json({ error: "Unknown operative" }, { status: 404 });
